...
_text = "<br />ВНИМАНИЕ: Снаряжение показанное ниже акутально только на момент старта миссии.<br />
...
if (count _mags > 0) then {
	_text = _text + "<br/>АММУНИЦИЯ [#КОЛЛИЧЕСТВО]:<br/>";
...
if !(backpack player == "") then {
	_text = _text + "<br/>РЮКЗАК [%ПОЛОН]:<br/>";
...
if (count _items > 0) then {
	_text = _text + "<br/>ПРЕДМЕТЫ [#КОЛЛИЧЕСТВО]:<br/>";
	...
	_text = _text + "<br/>*Обозначает экипированное снаряжение.";
};
...
player createDiaryRecord ["diary", ["Личное Снаряжние", _text]];